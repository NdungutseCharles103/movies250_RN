...
const TopSlider: React.FC<Props> = ({ tabs, active, setActive }) => {
	const theme = useTheme();

	return (
		<ScrollView style={[tw`h-8 my-2`]} horizontal showsHorizontalScrollIndicator={false}>
			{tabs.map((tab: Tab, i: number) => (
        <Pressable onPress={()=> setActive(tab.name)}>
				<CText key={i}
